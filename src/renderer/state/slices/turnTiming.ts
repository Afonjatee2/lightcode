import { isThreadTurnActive, type Thread, type ThreadStatus } from "@/shared/contracts";
import type { CompletedTurnRecord } from "./runtimeEventSlice";
import type { AppStoreState } from "./shared";

/**
 * Shared turn-timing helpers. Both the thread slice (terminal threads, driven by
 * the status channel) and the runtime-event slice (GUI threads, driven by the
 * ordered `turn.started`/`turn.completed` runtime events) compute turn windows
 * and frozen "Worked for X" records the same way.
 *
 * Callers are responsible for `markThreadRuntimeForPersistence` when a returned
 * `runtimeCompletedTurnsByThread` differs from the input — these helpers stay
 * free of that dependency so the module never imports runtime-slice values.
 */

/** Subset of store state needed to derive/append completed-turn records. */
type TurnTimingState = Pick<
  AppStoreState,
  "runtimeCompletedTurnsByThread" | "runtimeItemIdsByThread" | "runtimeItemsByIdByThread"
>;
...
export function parseTurnIso(iso: string | undefined): number | null {
  if (!iso) return null;
...
function resolveCompletedTurnAnchorItemId(state: TurnTimingState, threadId: string): string | null {
