...
import { markThreadRuntimeForPersistence } from "./runtimeEventSlice";
import type { SliceCreator } from "./shared";
import { appendCompletedTurnIfClosed, deriveTurnTiming, type TurnCloseUpdate } from "./turnTiming";
  ...
  updateThreadRuntime: (threadId, input) =>
    set((state) => {
      ...
      const threads: Thread[] = state.threads.map((thread): Thread => {
        ...
        const isGui = thread.presentationMode === "gui";
        let effectiveStatus = input.status;
        let effectiveAttention = input.attention;
        if (isGui) {
          // GUI working/idle is owned by the ordered `turn.completed` runtime
          // event (single source of truth), so the thread-state channel's
          // idle/finished is ignored here — a late idle can no longer race the
          // turn's trailing runtime events and strand a stale "working".
          // `forceCloseActiveTurn` is the one explicit exception: a synthetic
          // close for a failed optimistic launch/submit that will never produce
          // a real `turn.completed`.
          if (input.forceCloseActiveTurn && isThreadTurnActive(thread.status)) {
            effectiveStatus = isVisible ? "idle" : "finished";
            effectiveAttention = "none";
          } else if (
            (input.status === "idle" || input.status === "finished") &&
            (thread.status === "working" ||
              thread.status === "needs_approval" ||
              thread.status === "needs_reply")
          ) {
            // Only ignore the racing idle/finished when a turn is genuinely
            // mid-flight (the close is owned by `turn.completed`). Reconnect /
            // hydration / launch-ready transitions out of inactive/launching
      ...
      if (turnsChanged) markThreadRuntimeForPersistence(threadId);
      return {
        threads,
    ...
    set((state) => {
      ...
      const threads = state.threads.map((thread) => {
        ...
        if (snapshot) {
          ...
          const prevTurns = turnUpdate.runtimeCompletedTurnsByThread;
          turnUpdate = appendCompletedTurnIfClosed(
            { ...state, ...turnUpdate },
            thread.id,
            thread,
          ...
          if (turnUpdate.runtimeCompletedTurnsByThread !== prevTurns) {
            markThreadRuntimeForPersistence(thread.id);
          }
