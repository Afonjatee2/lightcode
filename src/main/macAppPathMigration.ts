...
function appNamesFor(channel: PoracodeChannel): { current: string; legacy: string[] } {
  return channel === "nightly"
    ? {
        current: "Tee's Cockpit Nightly.app",
        legacy: ["Poracode Nightly.app", "Lightcode Nightly.app"],
      }
    : { current: "Tee's Cockpit.app", legacy: ["Poracode.app", "Lightcode.app"] };
}
 ...
 * dead tile even though the renamed Tee's Cockpit bundle launches normally.
 *
 ...
 * before preparing later updates, so installs continue targeting the current app.
 */
export function repairLegacyMacAppPath(
  ...
  try {
    ...
    // The app remains launchable from its canonical Tee's Cockpit path if the
